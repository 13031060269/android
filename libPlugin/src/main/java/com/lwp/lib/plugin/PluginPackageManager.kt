...
class PluginPackageManager(private val orig: PackageManager) : PackageManager() {
    @Throws(NameNotFoundException::class)
    override fun getPackageInfo(packageName: String, flags: Int): PackageInfo {
        val plugin = PluginManager.getInstance().getPluginByPackageName(
            packageName
        )

        return plugin?.packageInfo ?: orig.getPackageInfo(packageName, flags)
    }
...
//         ApkPlugin.getInstance().getPluginByPackageName(packageName);
    ...
    override fun getPackageGids(packageName: String): IntArray {
        ...
        return if (plugin != null) {
            plugin.packageInfo.gids
        } else orig.getPackageGids(packageName)
    }
    ...
    override fun getApplicationInfo(packageName: String, flags: Int): ApplicationInfo {
        ...
        return if (plugin != null) {
            plugin.packageInfo.applicationInfo
        } else orig.getApplicationInfo(packageName, flags)
    }
...
//        if (plugin != null) {
//            val actClassName = component.className
...
//        }
    ...
    override fun resolveActivity(intent: Intent, flags: Int): ResolveInfo? {
        ...
        if (component != null) {
            ...
            if (plug != null) {
                plug.getActivity(intent)?.also { return@resolveActivity it }
            }
        }
    ...
    override fun getActivityIcon(activityName: ComponentName): Drawable? {
        val plug = PluginManager.getInstance().getPluginByPackageName(
            activityName.packageName
        )
        if (plug != null) {
            val actInfo = plug.findActivityByClassName(
                activityName
                    .className
            )
            val icon = actInfo!!.icon
            if (icon != 0) {
                return plug.resources?.getDrawable(icon, null)
            }
        }
    ...
    override fun getApplicationIcon(packageName: String): Drawable? {
        ...
        if (plug != null) {
            val appIcon = plug.packageInfo.applicationInfo.icon
            return if (appIcon != 0) {
                return plug.resources?.getDrawable(appIcon, null)
            } else {
                null
            }
        }
